...
Across all splitting strategies, `TextContentSplitter` keeps fences that fit within the chunk budget intact. Larger fenced blocks are split into bounded source slices and each slice receives the original opening fence and a matching closer. Indentation and code content are retained, including nested shorter fences. `GreedySplitter` applies the same bound before merging chunks. Rendered embedded image payloads are removed before splitting; alt text and literal code examples are retained. Embedding requests may split a batch into smaller batches, but an oversized individual input fails without truncation, preserving the previous stored page.
